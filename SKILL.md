...
```bash
pip install anthropic                # 默认 provider
pip install boto3                    # 仅 LLM_PROVIDER=bedrock 时需要
```
...
3. **P3 date-only (v5.2 fallback)**：P1/P2 都失败，但同日的酒店发票 + 水单 → 低置信匹配，报告里标 `⚠️ 仅日期匹配`
...
`~/invoices/{YYYY-QN}` 或 `~/invoices/{YYYY-MM}`。若目录已存在且非空，询问用户是覆盖还是另建（避免意外合并）。
